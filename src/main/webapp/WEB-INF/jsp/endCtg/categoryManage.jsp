...
<head>
	...
	<script type="text/javascript">
		...
		$(document).on("click", "#categoryForm .step_list.step01 li", function() {
			...
			if(useYn == 'Y')	$("#useYn").prop("checked",true);
			else 	$("#useYn").prop("checked",false);
			if(departCd == 'CS' ){
				$("#departCd1").prop("checked", true);
				$("#categoryForm [name='departCd']").val("CS");
			}else if (departCd == 'TM' ){
				$("#departCd2").prop("checked", true);
				$("#categoryForm [name='departCd']").val("TM");
			}
		...
		function selectCategory(ctgNum, step){
			...
			$.ajax({
				url : "<c:url value='/endCtg/selectCategoryAjax' />",
				data : {"ctgNum" : ctgNum},
				type : "post",
				success : function(result) {
					if(step == 1){
						...
						$.each(result, function(idx, item){
							//var li = '<li data-ctg-num="'+item.ctg_num+'" data-departCd="'+item.depart_cd+'"><span class="no">'+item.depart_cd+'</span><p class="text">'+item.ctg_nm+'</p>';
							var li = '<li data-useYn="'+item.use_yn+'" data-ctg-num="'+item.ctg_num+'" data-departCd="'+item.depart_cd+'"><p class="text">'+item.ctg_nm+'</p>';
							if (item.use_yn == 'N') {
						...
						$.each(result, function(idx, item){
							var ctg_num_reface;
							if(item.ctg_num == '9999999998') ctg_num_reface = "TM";
							else if(item.ctg_num == '9999999999') ctg_num_reface = "CS";
...
*/				
			...
			$.ajax({
				url : "<c:url value='/endCtg/insertCategory' />",
		...
		$(document).on("click", "#categoryForm .dev-update-btn", function() {
			...
			$.ajax({
				url : "<c:url value='/endCtg/updateCategory' />",
				data : $("#categoryForm").serialize(),
				type : "post",
				success : function(result) {
					if(result != null && result.rtnCd != null && result.rtnCd == "SUCCESS"){
		...
		$(document).on("click", "#categoryForm .dev-delete-btn", function() {
			...
			$.ajax({
				url : "<c:url value='/endCtg/deleteCategory' />",
				data : $("#categoryForm").serialize(),
				type : "post",
				success : function(result) {
					if(result != null && result.rtnCd != null && result.rtnCd == "SUCCESS"){
		...
		function goSearch(){
			$("#categoryForm").attr("action", "<c:url value='/endCtg/selectCategory' />");
			$("#categoryForm").submit();
		}
		...
		<div class="left_content_area">
			<form id="categoryForm" name="categoryForm" method="post" action="">
				...
				<div class="left_content_head">
					<h2 class="sub_tit">후처리 분류 설정</h2>
					<!-- <button type="button" class="btn_go_help">도움말</button>  -->
					<span class="left_top_text">후처리 분류를 설정합니다. 상담직원이 상담 후에 분류를 선택됩니다.</span>
				</div>
				 ...
				 -->
						...
						<div class="box_scroll">
							<ul class="step_list step01"><!-- 단계별 class명 추가 -->
								<c:forEach var="data" items="${categoryList }" varStatus="status">
									<li data-ctg-num="${data.ctg_num }" data-departCd="${data.depart_cd }" data-useYn="${data.use_yn}">
										<%-- <span class="no">${data.depart_cd }</span> --%>
						...
						<div class="box_scroll_bottom">
							<h4 class="sub_cont_tit">
								...
								<span style="float:right;">
									<input type="radio" class="radio_18 no_text" id="departCd1" name="departCdCheck" value="CS" <c:if test="${sessionVo.departCd eq 'CS' }">checked </c:if> />
									<label for="departCd1" style="margin-right: 10px;"><span></span>CS</label>
									<input type="radio" class="radio_18 no_text" id="departCd2" name="departCdCheck" value="TM" <c:if test="${sessionVo.departCd eq 'TM' }">checked </c:if> />
									<label for="departCd2"><span></span>TM</label>
								</span>
								<!--
								<span style="float:right;">
									<font style="font-size:14px; font-weight:100; padding-right:7px;">대고객용 :</font><input type="checkbox" class="checkbox_18" id="checkbox_01" name="cstmYnCheck" value="Y"><label for="checkbox_01"><span></span></label>
								</span>
								-->--%>
							</h4>
							<input type="text" name="ctgNm1" class="form_step_plus" placeholder="분류명을 입력하세요." maxlength="30">
							...
							<p class="text_info_star">추가 - 최대 30자리까지 입력 가능하며 최초 등록 시 "사용안함"으로 설정됩니다.</p>
							<p class="text_info_star">수정 - 원하는 대분류를 클릭한 후 분류명, 사용옵션 수정이 가능합니다. (부서 정보는 수정 불가)</p>
					...
					<div class="step_area dev-ctg-step" data-dev-depth="2">
						...
						<div class="box_scroll_bottom">
							...
							<input type="text" name="ctgNm2" class="form_step_plus" placeholder="분류명을 입력하세요." maxlength="30">
							<div class="btn_left_area">
								<button type="button" class="btn_step_plus dev-insert-btn"><i></i>추가</button>
								<button type="button" class="btn_step_modify dev-update-btn"><i></i>수정</button>
								<button type="button" class="btn_step_del dev-delete-btn"><i></i>삭제</button>
							...
							<p class="text_info_star">추가/수정 - 최대 30자리까지 입력 가능합니다.</p>
					...
					<div class="step_area dev-ctg-step dev-depth-3" data-dev-depth="3">
						...
						<div class="box_scroll_bottom">
							...
							<input type="text" name="ctgNm3" class="form_step_plus" placeholder="분류명을 입력하세요." maxlength="30">
							<div class="btn_left_area">
								<button type="button" class="btn_step_plus dev-insert-btn"><i></i>추가</button>
								<button type="button" class="btn_step_modify dev-update-btn"><i></i>수정</button>
								<button type="button" class="btn_step_del dev-delete-btn"><i></i>삭제</button>
					...
					<div class="step_bottom">
						<p class="text_info_star">부서 구분 및 전문 상담직원 배정을 위해 대/중분류가 사용되며, "사용" 설정된 대분류는 고객에게 노출되오니 주의바랍니다.</p>
