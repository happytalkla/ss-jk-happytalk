				...
				<table class="tCont setting">
					...
					<tbody>
						<tr>
							<th>제목</th>
							<td class="textL">
								<input type="text" name="title" value="${title }" maxlength="50" style="width:100%;"/>
							</td>
						</tr>
						<tr>
							<th>공지</th>
								...
								<c:choose>
									...
									<c:otherwise>
										<c:forEach var="data" items="${noticeDivList }" varStatus="status">
											<c:if test="${memberDivCd eq 'M' and data.cd eq 'C'}">
												<option value="${data.cd}">${data.cd_nm }</option>
											</c:if>

											<c:if test="${memberDivCd eq 'S' or memberDivCd eq 'A'}">
												<option value="${data.cd}">${data.cd_nm }</option>
											</c:if>
										</c:forEach>
									</c:otherwise>
